...
def random_table(self):
    ...
    you can read more about it on page 4 of Langton's paper."""
    rule_set = []
    for i in range(self.k ** (2 * self.r) + 1):
        g = np.random.rand()
...
def table_walk(self):
    ...
    to the automaton with a lower lambda."""
    power = self.k ** (2 * self.r + 1)
    rule_set = [0] * power
    changes = np.random.choice(power, int(self.lambda_param * power),
              replace = False)
...
class CASim(Model):
    ...
    def lambda_rule(self, val):
        """Setter for the lambda rule, clipping it between 0 and 1-(1/k)."""
        maximum = 1-(1/self.k)
